...
@section('page_title', 'Notifikasi Anda') 
...
<div class="container-fluid py-4">
    ...
    @else
        ...
        {{-- TOMBOL TANDAI SEMUA SUDAH DIBACA (DIPINDAHKAN KE ATAS TANPA PEMBATAS GARIS) --}}
        <div class="d-flex justify-content-end mb-4">
            <form action="{{ route('wali.notifikasi.mark_all_read') }}" method="POST" class="form-mark-all">
                @csrf
        ...
        <div class="list-group list-group-notification">
            @foreach($notifikasis as $notifikasi)
                ...
                <a href="{{ $notifikasi->link ?? '#' }}" 
                   class="list-group-item list-group-item-action mb-2 rounded-4 p-3 {{ $readClass }}"
                   onclick="event.preventDefault(); document.getElementById('read-form-{{ $notifikasi->id }}').submit();">
                    
                    <div class="d-flex w-100 justify-content-between align-items-start">
                        <div class="d-flex align-items-center flex-grow-1 me-3">
                            {{-- Ikon Notifikasi --}}
                            <i class="{{ $icon }} me-3 fs-5 {{ $iconClass }} flex-shrink-0"></i>
                            ...
                            <div>
                                ...
                                <p class="mb-1 small text-muted text-wrap description-text">{{ Str::limit($notifikasi->body, 80) }}</p>
                            </div>
                        </div>
                        
                        {{-- Status dan Waktu --}}
                        <div class="text-end flex-shrink-0">
                            <small class="text-muted d-block mb-1">{{ $notifikasi->created_at->diffForHumans() }}</small>
